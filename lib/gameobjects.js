...
World.prototype.update_state = function(t, dt) {
  ...
  for (var player_id in players) {
    ...
    if (!player.dead) {
      ...
      if (!player.is(SHIELD) && player.is(SHOOT) && ship.can_fire(dt)) {
        ship.set(SHOOT, true);
...
Ship.prototype.can_fire = function(dt) {
	return !this.reload_time && this.energy >= this.weapon.shoot_cost * dt;
};

Ship.prototype.fire = function(World) {
  // powerup should be moved to ship for now
  var powerup   = powerup || this.powerup,
  	weapon      = this.weapon,
  	bullets     = weapon.get_bullets(World),
  	energy_cost = weapon.get_energy_cost(World),
  	i = 0;
  ...
  this.reload_time = weapon.get_reload_time(World);
  return bullets;
};
...
CannonWeapon = Weapon;
CannonWeapon.prototype = new Weapon();

CannonWeapon.prototype.bullet_acceleration_speed = 1;
CannonWeapon.prototype.bullet_max_speed          = 250;
CannonWeapon.prototype.bullet_lifetime           = 300;
CannonWeapon.prototype.reload_time               = 15; 
CannonWeapon.prototype.shoot_cost                = 300;
CannonWeapon.prototype.sound                     = 'bullet_spawn';

CannonWeapon.prototype.get_bullets = function(World) {
  var ship = this.ship,
...
CannonWeapon.prototype.get_energy_cost = function(World) {
  var ship = this.ship,
      powerup = ship.powerup,
      energy_cost = 0;
  
  if ((powerup & POWERUP_RAPID) == POWERUP_RAPID) {
    energy_cost = (this.shoot_cost / 3) * World.delta;
  } else {
    energy_cost = this.shoot_cost * World.delta;
  }
  
  return energy_cost;
};

CannonWeapon.prototype.get_reload_time = function(World) { 
  var ship = this.ship,
      powerup = ship.powerup,
      reload_time = 0;
  
  if ((powerup & POWERUP_RAPID) == POWERUP_RAPID) {
    reload_time = World.tick + (this.reload_time / 2) * World.delta;
  } else {
    reload_time = World.tick + this.reload_time * World.delta;
  }
...
var RapidFireUpgrade = function(weapon) {
  this.weapon = weapon;
};
RapidFireUpgrade.type = POWERUP_RAPID;
...
try { 
  ...
  global.CannonWeapon = CannonWeapon;
