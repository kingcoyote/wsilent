...
var SHIP_WIDTH = 9,
    ...
    BULLET_ACCELERATION_SPEED = 1, // WEAPON
    BULLET_MAX_SPEED = 250, // WEAPON
    BULLET_LIFETIME = 300, // WEAPON
    RBULLET_LIFETIME = 250; // WEAPON

var DEATH_CAUSE_KILLED = 1,
    DEATH_CAUSE_SUICDE = 2;

// Power up codes
var POWERUP_SPREAD  = 1, // WEAPON
    POWERUP_RAPID   = 2, // WEAPON
    POWERUP_RICO    = 4; // WEAPON
 ...
 */
World.prototype.on_player_fire = function(player, angle) {}; // WEAPON
...
World.prototype.set_state = function(state, players, powerups) {
  ...
  for (var i = 0; i < powerups.length; i++) { // WEAPON
 ...
 *  Spawns a new bullet.
 */
World.prototype.fire_player_cannon = function(player_id, angle, pos, vel, powerup) { // WEAPON
  var player = this.players[player_id];
...
World.prototype.update_state = function(t, dt) {
  ...
  for (var player_id in players) {
    ...
    if (!player.dead) {
      ...
      if (player.reload_time && t >= player.reload_time) { // WEAPON
        player.reload_time = 0;
      }

      if (!player.is(SHIELD) && player.is(SHOOT) && !player.reload_time &&
                                        player.energy >= rules.shoot_cost * dt) { // WEAPON
  ...
  while (index--) {
    ...
    if (entity.destroyed) {
      switch (entity.type) {
        ...
        default:
          this.remove_entity(entity.id); // WEAPON
          break;
      }
    }
  }
...
function Player(initial) {
  ...
  this.powerup        = initial.powerup || 0; // WEAPON
  this.reload_time    = initial.reload_time || 0; // WEAPON
  this.respawn_time   = initial.respawn_time || 0;
  this.dead           = initial.dead || true;
  this.powerup_timers = initial.powerup_times || { 1: null, 2: null, 4: null}; // WEAPON
  this.on_after_init();
}
...
Player.prototype.get_repr = function() {
  var repr = {
    ...
    energy: this.energy,
    action: this.action,
    powerup: this.powerup, // WEAPON
    ready: this.ready,
    reload_time: this.reload_time, // WEAPON
    respawn_time: this.respawn_time, // WEAPON
    dead: this.dead,
    powerup_timers: this.powerup_timers // WEAPON
  };
 ...
 */
function Bullet(initial) { // WEAPON
  this.on_before_init();
 ...
 */
Bullet.prototype.get_bounds = function() { // WEAPON
  return {
    x: this.pos[0] - 5,
    y: this.pos[1] - 5,
    w: 10,
 ...
 */
function Powerup(initial) { // WEAPON
  this.on_before_init();
 ...
 *  @return {undefined} Nothing.
 */
Powerup.prototype.destroy = function(destroyed_by) { // WEAPON
  this.destroyed = true;
 ...
 *  @return {x, y, w, h} The bounds of the Ship.
 */
Powerup.prototype.get_bounds = function(expand) { // WEAPON
  return {
    x: this.pos[0] - 5,
    y: this.pos[1] - 5,
    w: 5,
...
function get_random_powerup_type() { // WEAPON
...
function get_powerup_decline_time(rules, powerup) { // WEAPON
