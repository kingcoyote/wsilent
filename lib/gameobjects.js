...
World.prototype.fire_player_cannon = function(player_id, angle, pos, vel, powerup) { 
  ...
  var lifetime = (powerup & POWERUP_RICO) == POWERUP_RICO ?
                                  ...
                                  this.tick + (ship.weapon.bullet_lifetime *  this.delta); // WPN
  ...
  var vel = vel || [
    Math.cos(angle - Math.PI / 2) * (ship.weapon.bullet_max_speed) + ship.vel[0], // WPN
    Math.sin(angle - Math.PI / 2) * (ship.weapon.bullet_max_speed) + ship.vel[1]  // WPN
  ];
  ...
  if ((powerup & POWERUP_RAPID) == POWERUP_RAPID) {
    player.energy -= (ship.weapon.shoot_cost / 3) * this.delta;
    player.reload_time = this.tick + (ship.weapon.reload_time / 2) * this.delta; // WPN
  } else {
    player.energy -= ship.weapon.shoot_cost * this.delta;
    player.reload_time = this.tick + ship.weapon.reload_time * this.delta; // WPN
  }
...
World.prototype.update_state = function(t, dt) {
  ...
  for (var player_id in players) {
    ...
    if (!player.dead) {
      ...
      if (!player.is(SHIELD) && player.is(SHOOT) 
          && !player.reload_time && player.energy >= player.entity.weapon.shoot_cost * dt) { // WPN
...
function CannonWeapon(initial) { // WEAPON
  this.bullet_acceleration_speed = 1;
  this.bullet_max_speed          = 250;
  this.bullet_lifetime           = 300;
  this.reload_time               = 15; 
  this.shoot_cost                = 30;
  this.sound                     = 'bullet_spawn';
}

extend_class(CannonWeapon, EntityBase);
